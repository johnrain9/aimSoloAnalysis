...
from analytics.trackside import rank, rules


class TestTracksideRules(unittest.TestCase):
    def _insights_for(self, target, reference=None, segment_extra=None):
        segment = {
            "segment_id": "seg-1",
            "corner_id": "T1",
            "target": target,
        ...
        return rules.generate_insights([segment], "ref")

    def _rule_ids(self, insights):
        return [item["rule_id"] for item in insights]

    def test_early_braking_rule(self):
        insights = self._insights_for(
            {"segment_time_delta_s": 0.09, "brake_point_dist_m": 90.0},
            {"brake_point_dist_m": 105.0},
        )
        self.assertIn("early_braking", self._rule_ids(insights))

        no_time_loss = self._insights_for(
            {"segment_time_delta_s": 0.04, "brake_point_dist_m": 90.0},
            {"brake_point_dist_m": 105.0},
        )
        self.assertNotIn("early_braking", self._rule_ids(no_time_loss))

        not_early = self._insights_for(
            {"segment_time_delta_s": 0.09, "brake_point_dist_m": 98.0},
            {"brake_point_dist_m": 105.0},
        )
        self.assertNotIn("early_braking", self._rule_ids(not_early))
    ...
    def test_late_throttle_pickup_rule_distance(self):
        ...
        self.assertIn("late_throttle_pickup", self._rule_ids(insights))

        good = self._insights_for(
            {"segment_time_delta_s": 0.1, "throttle_pickup_dist_m": 34.0},
            {"throttle_pickup_dist_m": 28.0},
        )
        self.assertNotIn("late_throttle_pickup", self._rule_ids(good))
    ...
    def test_late_throttle_pickup_rule_proxy_time(self):
        ...
        rule_ids = self._rule_ids(insights)
        self.assertIn("late_throttle_pickup", rule_ids)
        pickup = next(item for item in insights if item["rule_id"] == "late_throttle_pickup")
    ...
    def test_neutral_throttle_rule_time(self):
        ...
        self.assertIn("neutral_throttle", self._rule_ids(insights))

        speed_not_flat = self._insights_for(
            {
                "segment_time_delta_s": 0.08,
                "neutral_throttle_s": 1.1,
        ...
        self.assertNotIn("neutral_throttle", self._rule_ids(speed_not_flat))
    ...
    def test_neutral_throttle_rule_distance(self):
        ...
        self.assertNotIn("neutral_throttle", self._rule_ids(no_time_loss))
    ...
    def test_line_inconsistency_rule(self):
        ...
        self.assertIn("line_inconsistency", self._rule_ids(insights))

        delta_trigger = self._insights_for(
            {"segment_time_delta_s": 0.05, "line_stddev_m": 1.4},
            {"line_stddev_m": 0.7},
        )
        self.assertIn("line_inconsistency", self._rule_ids(delta_trigger))

        good = self._insights_for(
            {"segment_time_delta_s": 0.05, "line_stddev_m": 1.2},
            {"line_stddev_m": 0.9},
        )
        self.assertNotIn("line_inconsistency", self._rule_ids(good))
    ...
    def test_corner_speed_loss_rule(self):
        ...
        self.assertIn("corner_speed_loss", self._rule_ids(insights))

        good = self._insights_for(
            {"segment_time_delta_s": 0.05, "min_speed_kmh": 72.5},
            {"min_speed_kmh": 74.0},
        )
        self.assertNotIn("corner_speed_loss", self._rule_ids(good))
    ...
    def test_exit_speed_rule_proxy(self):
        ...
        self.assertIn("exit_speed", self._rule_ids(insights))
        exit_speed = next(item for item in insights if item["rule_id"] == "exit_speed")
        self.assertAlmostEqual(0.4, exit_speed["confidence"], places=3)
        self.assertEqual("low", exit_speed["confidence_label"])

        accel_not_low = self._insights_for(
            {
                "segment_time_delta_s": 0.05,
                "exit_speed_30m_kmh": 90.0,
        ...
        self.assertNotIn("exit_speed", self._rule_ids(accel_not_low))
    ...
    def test_entry_speed_rule(self):
        ...
        self.assertIn("entry_speed", self._rule_ids(insights))

        late_brake = self._insights_for(
            {"segment_time_delta_s": 0.05, "entry_speed_kmh": 100.0, "brake_point_dist_m": 111.0},
            {"entry_speed_kmh": 104.0, "brake_point_dist_m": 100.0},
        )
        self.assertNotIn("entry_speed", self._rule_ids(late_brake))
    ...
    def test_steering_smoothness_rule(self):
        ...
        self.assertIn("steering_smoothness", self._rule_ids(insights))

        yaw_ok = self._insights_for(
            {"segment_time_delta_s": 0.05, "yaw_rms": 1.15, "min_speed_kmh": 90.0},
            {"yaw_rms": 1.0, "min_speed_kmh": 92.5},
        )
        self.assertNotIn("steering_smoothness", self._rule_ids(yaw_ok))
