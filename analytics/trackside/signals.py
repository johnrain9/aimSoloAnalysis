"""Trackside signal extraction for per-segment comparisons.

Signals are deterministic rule hits without final wording. They are intended
to feed a synthesis layer that resolves conflicts and emits a single insight.
"""
...
def generate_signals(
    ...
    """Generate per-segment signals from segment comparisons."""
    ...
    for segment in segments:
        ...
        signals.extend(
            _signal_early_braking(segment, target, reference, quality, time_gain_s, comparison_label, common)
        )
        signals.extend(
            _signal_late_throttle(segment, target, reference, quality, time_gain_s, comparison_label, common)
        )
        signals.extend(
            _signal_neutral_throttle(segment, target, reference, quality, time_gain_s, comparison_label, common)
        )
        signals.extend(
            _signal_line_inconsistency(segment, target, reference, quality, time_gain_s, comparison_label, common)
        )
        signals.extend(
            _signal_corner_speed_loss(segment, target, reference, quality, time_gain_s, comparison_label, common)
        )
        signals.extend(
            _signal_exit_speed(segment, target, reference, quality, time_gain_s, comparison_label, common)
        )
        signals.extend(
            _signal_entry_speed(segment, target, reference, quality, time_gain_s, comparison_label, common)
        )
        signals.extend(
            _signal_steering_smoothness(segment, target, reference, quality, time_gain_s, comparison_label, common)
        )

    return [item.to_dict() for item in signals]


def _signal_early_braking(
...
def _signal_line_inconsistency(
    segment: Dict[str, Any],
    target: Dict[str, Any],
    reference: Dict[str, Any],
    quality: Dict[str, Any],
...
def _signal_steering_smoothness(
    segment: Dict[str, Any],
    target: Dict[str, Any],
    reference: Dict[str, Any],
    quality: Dict[str, Any],
